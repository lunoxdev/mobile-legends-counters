    ...
    <section className="w-full h-screen px-2">
        ...
        {roles.map((role) => (
          <Button
            ...
            className={`px-5 py-3 border-b-[0.1px] outline-none border-[#7890B3] text-[#7890B3] hover:text-[#f2f2f2] hover:bg-gradient-to-t hover:from-[#5C67B8] hover:to-[#5C67B8]/5 ${
              activeRoleId === role.id ? "border-b-2 border-[#98FFFF] bg-gradient-to-t from-[#5C67B8] to-[#5C67B8]/5 text-[#f2f2f2]" : ""
            }`}
          >
            {role.name}
          </Button>
      ...
      <Card className="grid grid-cols-7 lg:grid-cols-10 h-auto gap-3 py-4">
        {heroes.map((hero) => (
          <div
            key={hero.heroid}
            className="relative overflow-hidden h-auto rounded-lg hover:shadow-md hover:shadow-blue-500"
          >
            <Image
              src={hero.key}
              alt={hero.name}
            ...
            />
            <CardFooter className="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-[#292E52] via-[#292E52]/70 to-transparent px-2 pt-10 text-sm justify-center">
              {hero.name}
            </CardFooter>
          </div>
        ))}
